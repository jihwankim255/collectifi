...
const MyPage = () => {
  ...
  useEffect(() => {
    ...
    if (userInfo && userInfo.user.token_amount) setAmount(userInfo.user.token_amount);
    return () => {
      userRefresh();
      cardRefresh();
      postRefresh();
  ...
  const infoData = [
    userInfo.user.address.slice(0, 10),
      ...
      >
        <CardList itemWidth={cardWidth} key={0}>
          {cardList.map((el: myCard, i: number) => {
            ...
            else if (el.Nft_galleries.length > 0) info = 'ON DISPLAY';
            return (
              <CardListItem key={i} info={info} linkTo={`/market/${el.token_id}`}>
                <PlayerCard
                  imgSrc={el.img_url}
                  //cardWidth={cardWidth}
                  //glow={Glow.orange}
          ...
          {gallList.map((el: myGallery, i: number) => {
            ...
            const pCard = (
              <div className="player-wrap">
                <PlayerCard imgSrc={el.img_url} />
              </div>
            );
            const gallImg = (
              <div className="gallimg-wrap">
                <img src={galEl.img_url} alt="gallery" />
              </div>
            );
            const listItem = [
              gallImg,
          ...
          >
            {new Array(10).fill(0).map((el, i) => {
              const listItem = [
                ...
                'test3',
                'test4',
                <a key={0} onClick={() => console.log(11)}>
                  aa
                </a>,
              ];
              return (
