...
import {userAmount, userNickname, userReferral} from '../../atom';
import {editNicknameQuery, editReferralQuery} from '../../api/mypage/atom';
...
const EditInfo = () => {
  ...
  const editNickName = useRecoilCallback(({set, snapshot}) => async () => {
    ...
    const queryResult = await snapshot.getPromise(
      editNicknameQuery(nickEditInputRef.current.value),
    );
  ...
  const editReferral = useRecoilCallback(({set, snapshot}) => async () => {
    ...
    const queryResult = await snapshot.getPromise(
      editReferralQuery(referralEditInputRef.current.value),
    );
    ...
    <EditInfoLayout>
      ...
      {isEditClick ? (
        <div className="open-edit-wrap">
          <div>
            <ThinButton color="rgb(250, 250, 250)" onClick={handleEditOkClick}>
              OK
            </ThinButton>
          </div>
          <div>
        ...
        !isReferClick && (
          <ThinButton color="rgb(250, 250, 250)" onClick={handleEditClick}>
            EDIT
          </ThinButton>
        )
      )}
      ...
      {(!referral || referral === '') &&
        (isReferClick ? (
          <div className="open-edit-wrap">
            <div>
              <ThinButton color="rgb(250, 250, 250)" onClick={handleReffClick}>
                OK
              </ThinButton>
            </div>
            <div>
              <div className="referral-edit-wrap">
                ...
                <ThinInput
                  id="referral-edit-input"
                  placeholder="REFERRAL ADDR"
                  ref={referralEditInputRef}
                />
              </div>
            </div>
          </div>
          ...
          !isEditClick && (
            <ThinButton color="rgb(250, 250, 250)" onClick={handleReferClick}>
              REFERRAL
            </ThinButton>
          )
        ))}
