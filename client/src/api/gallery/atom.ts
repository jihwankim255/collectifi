...
import {galleries, cardByGallId, addCardByGallId, stake, addCardRegi} from '.';
...
export const addCardRegiQuery = selectorFamily({
  ...
  get:
    (param: {gallery_id: number; nft_id: number; nft_end_time: string}) =>
    async ({get}) => {
      get(userId);
...
export const getGallByIdQuery = selectorFamily({
  ...
  get:
    ...
    async ({get}) => {
      ...
      if (!response) return null;
      return response.data.gallerys[gallId - 1];
    },
});
